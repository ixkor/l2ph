...
resourcestring
  ...
  rsFailedLocalServer = 'Не удалось зарегистрировать локальный сервер на порте %d'+ #13#10+ 'Возможно этот порт занят другим приложением';
  rsCreateNewConnect = 'Создано новое соединение - %d';
  rsConnectBreak = 'Соединение %d разорвано';
...
begin
  ...
  if ExitCode <> 0 then
  begin
    SendMSG('На сокете: '+inttostr(hsocket)+' ошибка: '+inttostr(ExitCode)+' '+SysErrorMessage(ExitCode));
  end;
  // Закрываем сокет
